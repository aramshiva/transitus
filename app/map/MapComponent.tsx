...
function VehiclePopup({
  ...
  const deviationColor =
    vehicle.tripStatus?.scheduleDeviation === 0
      ? "text-green-600"
      : Math.abs(vehicle.tripStatus?.scheduleDeviation || 0) > 300
        ? "text-red-600"
        : "text-yellow-600";
          ...
          {vehicle.tripStatus.occupancyCount !== undefined &&
            ...
            vehicle.tripStatus.occupancyCapacity > 0 && (
              <div className="text-sm">
                ...
                {vehicle.tripStatus.occupancyCount}/
                {vehicle.tripStatus.occupancyCapacity} (
                {Math.round(
                  (vehicle.tripStatus.occupancyCount /
                    vehicle.tripStatus.occupancyCapacity) *
                    100,
              ...
              <div className="text-sm">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  {vehicle.tripStatus.distanceAlongTrip !== undefined && (
                    <div>
                      <span className="font-medium">Distance Along Trip:</span>{" "}
                      {formatDistance(vehicle.tripStatus.distanceAlongTrip)}
                    </div>
                  )}
                  {vehicle.tripStatus.totalDistanceAlongTrip !== undefined && (
                    <div>
                      <span className="font-medium">Total Trip Distance:</span>{" "}
                      {formatDistance(
                        vehicle.tripStatus.totalDistanceAlongTrip,
                      )}
                    </div>
                  )}
                  {vehicle.tripStatus.closestStop && (
                    <div>
                      <span className="font-medium">Closest Stop:</span>{" "}
                      {vehicle.tripStatus.closestStop}
                    </div>
                  )}
                  {vehicle.tripStatus.situationIds &&
                    vehicle.tripStatus.situationIds.length > 0 && (
                      <div>
                        <span className="font-medium">Alerts:</span>{" "}
                        {vehicle.tripStatus.situationIds.join(", ")}
                      </div>
                    )}
                  {vehicle.location && (
  ...
  useEffect(() => {
    const fetchAllAgencies = async () => {
      const uniqueAgencyIds = Array.from(
        new Set(vehicles.map((v) => v.agency?.id).filter(Boolean)),
      ) as string[];
