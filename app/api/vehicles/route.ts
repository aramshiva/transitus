...
export async function GET() {
  try {
    ...
    const agencyLookup = agencyRefs.reduce((acc, agency) => {
    ...
    const vehiclePromises = agencies.map(async (agency) => {
      try {
        ...
        if (!response.ok) {
          console.warn(`Failed to fetch vehicles for agency ${agency.agencyId}: ${response.status}`)
        ...
        console.warn(`Error fetching vehicles for agency ${agency.agencyId}:`, error)
        return { vehicles: [], agencyId: agency.agencyId }
      }
    })
    ...
    const latestTime = Math.max(...results.map(r => r.currentTime || 0).filter(t => t > 0))
