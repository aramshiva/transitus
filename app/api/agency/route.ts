...
export async function GET(request: NextRequest) {
  try {
    ...
    const baseUrl = process.env.ONEBUSAWAY_BASE_URL || 'https://api.pugetsound.onebusaway.org'
    ...
    if (!response.ok) {
      throw new Error(`OneBusAway API error: ${response.status} ${response.statusText}`)
    }
